...
c     why only Sp=1=S case ??? 
      ...
      if ((Sp .eq. 1) .and. (S .eq. 1)) then
          hold(1,1,1,1)=2.0*factor*Az
          hold(1,-1,1,-1)=-2.d0*factor*Az
          hold(1,0,1,1)=-2.d0*factor*Aplus
          hold(1,1,1,0)=2.0*factor*Aminus   
          hold(1,-1,1,0)=-2.0*factor*Aplus
          hold(1,0,1,-1)=2.0*factor*Aminus
      end if

c     if ((Sp .eq. 1) .and. (S .eq. 0)) then
c         hold(0,0,1,-1)=factor*Aplus
c         hold(0,0,1,0)=factor*Az
c         hold(0,0,1,1)=factor*Aminus
c     end if

c     if ((Sp .eq. 0) .and. (S .eq. 1)) then
c         hold(1,-1,0,0)=factor*Aminus
c         hold(1,0,0,0)=factor*Az
c         hold(1,1,0,0)=factor*Aplus
c     end if
